...
import (
	...
	"github.com/shun57/go_todo_app/clock"
	"github.com/shun57/go_todo_app/entity"
	"github.com/shun57/go_todo_app/testutil/fixture"
)
...
func TestJWTer_GenerateTok(t *testing.T) {
...
func TestJWTer_GetToken(t *testing.T) {
	...
	want, err := jwt.NewBuilder().
		...
		Issuer(`github.com/shun57/go_todo_app`).
	...
	sigend, err := jwt.Sign(want, jwt.WithKey(jwa.RS256, pkey))
	if err != nil {
		t.Fatal(err)
	}
	...
	req := httptest.NewRequest(
		http.MethodGet,
		`https://github.com/shun57`,
		nil,
	)

	req.Header.Set(`Authorization`, fmt.Sprintf(`Bearer %s`, sigend))
